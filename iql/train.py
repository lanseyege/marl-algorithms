    ...
    for temp_e in range(20):
        ...
        while not terminated:
            ...
            for agent_id in range(n_agents):
                ...
                action = agent_list[agent_id].select_action(cats, avail_actions, 0.05)
...
def main(args):
    ...
    for e in range(n_episodes):
        ...
        is_test = False
        while not terminated:
            if index_e > args.epsilon_decay_:
                epsilon = 0.05
            else:
                epsilon = epsilon - 0.95/args.epsilon_decay_ 
            index_e += 1
            #print("obs ... ")
            #print(obs)
            #print("state ... ")
            ...
            for agent_id in range(n_agents):
                ...
                action = agent_list[agent_id].select_action(cats, avail_actions, epsilon)
            ...
            if 'battle_won' in what and what['battle_won']:
                print('won ...')
            if 'dead_enemies' in what and what['dead_enemies'] > 0:
                print('dead enemies %d'% what['dead_enemies'])
            obs, state = env.get_obs(), env.get_state()
            obs = torch.FloatTensor(obs)
            state = torch.FloatTensor(state)
            for agent_id in range(n_agents):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epsilon_decay_', default=epsilon_decay_, type=float, help='epsilon decay')
